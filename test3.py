...
class Ball:
    ...
    def toggle_attachment(self, ropes):
        ...
        if self.is_attached:
            ...
            score += 10  # Increment score by 10 when the ball attaches to a rope
...
game_over_alpha = 0  # Transparency level (alpha) for fade-in effect
game_over_font = pygame.font.Font(None, 100)  # Large font size for Game Over text
...
while running:
    ...
    if game_over:
        # Gradually increase the transparency of the "Game Over" message
        if game_over_alpha < 255:
            game_over_alpha += 5  # Increase alpha for fade-in effect
        game_over_text.set_alpha(game_over_alpha)  # Apply updated alpha

        screen.blit(game_over_text, (WIDTH // 3, HEIGHT // 3))  # Display "Game Over" text
        pygame.display.flip()

        # Only stop the game when the animation is done
        if game_over_alpha >= 255:
    ...
    screen.blit(score_text, (1, 1))  # Display score at the top-left corner
